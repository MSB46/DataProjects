...
def collect_content(dataframe: pd.DataFrame):
    dataframe["content"] = dataframe.apply(
        ...
        f'{" ".join([x["secondary_artists"].lower()] * (1 if x["secondary_artists"] else 0))} '
        ...
        f'{" ".join([x["track_genre"]] * (14 if x["track_genre"] else 0))} '
...
class Recommendation:
    ...
    def recommend(
            self, max_songs=10, dist_factor=0.9, find_other_artists=False, simple=True, use_tfidf=0
    ):

        self.df = self.df[self.df["track_genre"].isin(set(self.provided_songs_df["track_genre"].values))]
        ...
        indices = np.argsort(distances)[:, :min(300, self.df.shape[0]) + len(self.id)]
